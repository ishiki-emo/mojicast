"""実行時パス解決（開発実行 / PyInstaller凍結 両対応・Windows / macOS 両対応）

- BASE: アセット(overlay.html, ui/, silero_vad.onnx, defaults/)の基準ディレクトリ。
        開発時=このファイルのフォルダ / Windows凍結=exeのフォルダ /
        mac凍結=.appバンドル内の Contents/Resources（読み取り専用）。
- DATA_BASE: 書き込みデータ(data/, logs/, models/)の基準ディレクトリ。
        Windows凍結では BASE と同じ（ポータブル運用）。mac凍結では
        ~/Library/Application Support/Mojicast（.appの中には書かない。
        dmg配布では .app が読み取り専用の場所に置かれうるため）。
- 凍結時は Hugging Face キャッシュを DATA_BASE/models に固定する。
  ・同梱版      : models/ にモデルが入った状態で配布 → 各ローダがローカルから読む（DL無し）
  ・軽量版(非同梱): models/ が空 → 各ローダのローカル優先読み込みが失敗し初回だけDL、
                    ダウンロード先も同じ models/ なので初回以降は自己完結する。
  ローカル優先読み込み（local_files_only→失敗時DL）は各モジュール側で行うため、
  ここで OFFLINE を強制はしない（＝新しいモデルの追加DLも妨げない）。

Windows凍結時は、pywebview(clr) を読み込む前に MOTW(ダウンロード印) を自己除去する。
...
if getattr(sys, "frozen", False):
    if sys.platform == "darwin":
        # .app バンドル: 実行ファイルは Mojicast.app/Contents/MacOS/ にある
        _contents = os.path.dirname(os.path.dirname(sys.executable))
        BASE = os.path.join(_contents, "Resources")
        DATA_BASE = os.path.join(
            os.path.expanduser("~/Library/Application Support"), "Mojicast")
        os.makedirs(DATA_BASE, exist_ok=True)
    else:
        BASE = os.path.dirname(sys.executable)
        DATA_BASE = BASE              # exe 隣に集約（ポータブル運用）
