...
![字幕オーバーレイの表示例](images/overlay_collab.png)
...
音声認識・句読点付け・英訳・コラボ相手の字幕化まで、**すべてお使いのPCの中だけで動きます**。
音声がネットに送られることはありません。
...
| ○ 実用 | Intel 12〜14世代 / Core Ultra | 通常配信は問題なし。コラボで2人とも喋りっぱなしだと負荷高め |
| △ 動くが重め | Intel 8〜11世代 / Ryzen 2000〜5000 | ソロ配信なら実用。[9章](#9-困ったときは)の軽量化設定を推奨 |
| ✕ 非推奨 | AVX2非対応の古いCPU・N100等の省電力CPU | リアルタイム認識が追いつきません |

最近のAMD（Zen 4以降）は int8 推論専用の命令（AVX-512 VNNI）を使えるため、
同価格帯のIntelより認識処理が数倍速く走ります。
...
| 右カラム | 文字スタイル・レイアウト・単語プロファイル・1対1コラボ・OBS連携 |
...
| タブ | 役割 |
|---|---|
| 認識させる単語 | 固有名詞の誤認識対策。「読み」欄には誤変換の形も入れられる。次回開始時に反映 |
...
字幕ログ上の「英訳」チップをオンにすると、確定字幕の下に英訳が併記されます。
翻訳もローカルで動作（ネット不要）。「配信→stream」「スパチャ→Super Chat」など
配信用語は組み込み辞書で対応済み。人名などは「英訳を固定する単語」で訳を固定できます。
...
| 「句読点/英訳の読み込みに失敗」と出る | 字幕本体は動きます。原因が `logs\load_error.log` に残るので報告時に添付を |
