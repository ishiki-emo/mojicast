...
編集はコックピットの「🎨 字幕スタジオ」→ 左メニュー「字幕」の「文字スタイル」「レイアウト」から。
...
**表示モード（v0.8.0〜）**: 「日本語と併記（標準）」と「**翻訳のみ表示**」を選べます。

- 翻訳のみでは訳文が**本文のサイズ・エフェクト**で表示され、日本語は出ません。海外リスナー向けの
  配信で画面の圧迫を防げます（上記のサイズ比・濃さ等の併記用の設定は使われません）
- 表示行数はレイアウトの`表示行数`がそのまま効きます（**2行**くらいが画面に馴染みやすい）
- デザインごとの設定なので、「英語配信」シーンを作っておけばワンタップ／ボイスコマンドで
  併記⇄翻訳のみを一括切替できます。翻訳がOFFのときは自動で併記（日本語表示）に戻ります
...
- **一定時間で字幕を消す**: オンにすると、確定した行が指定秒数のあと自動でフェードアウトします。
  「最新の1行だけ表示」のような使い方で、しばらく黙っても字幕が残り続けないようにしたいときに（既定はオフ＝従来どおり残る）
...
1. 文字スタイル／レイアウト編集画面の下部にある「📤 エクスポート」を押す
   （スタジオ左メニューの「保存・共有 → 📤 カスタマイズを書き出す」からも同じことができます）
2. 書き出す範囲を選ぶ:
   ...
   | 使用中の組み合わせ | 今使っている文字スタイル＋レイアウトの組 | セット配布・丸ごと引っ越し |
...
2. 編集画面下部の「📥 インポート」→ ファイルを選ぶ
   （スタジオ「保存・共有 → 📥 もらったカスタマイズを使う」からでもOK）
