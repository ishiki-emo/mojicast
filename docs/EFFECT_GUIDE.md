...
- 上部の「編集する単語セット」で**単語セット**（雑談用・ゲーム用・歌枠用…）を分けられます。
  単語セットは「共通」への加算で、同じ単語は単語セット側が優先
...
- **翻訳のみ表示（v0.8.0〜）でもエフェクトは使えます**。訳文に出る表記（例: Stella）でエフェクト
  単語を登録してください。人名は「英訳を固定する単語」に同じ表記で登録しておくと、訳のゆれが
  なくなり確実に発動します（英単語は単語の区切りを見て判定するので、短い単語でも誤発動しません）
