...
- 上部の「編集する単語セット」で**プロファイル**（雑談用・ゲーム用・歌枠用…）を分けられます。
  プロファイルは「共通」への加算で、同じ単語はプロファイル側が優先
...
- 「今日は落ち着いた配信にしたい」という日は、コックピットの字幕ログ上にある
  「**単語エフェクト**」チップをオフに。登録を消さずに、表示だけまとめて無効化できます
